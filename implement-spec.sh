...
MACOS_COMPAT_REMINDER="IMPORTANT: All shell scripts (if any) must work on macOS bash 3.2 AND Linux bash 5+. Do NOT use bash 4+ features: readarray/mapfile, declare -A (associative arrays), \${var,,}/\${var^^} (case conversion), |& (pipe stderr), declare -n/local -n (namerefs). You are running on Linux but this code MUST also run on macOS."
...
7. Commit your chages with a commit message starting with 'chunk N/T: ' followed by a brief description.
...
2. Update the spec file: change its State: field to IMPLEMENTED and add it to the spec index in specs/README.md.
3. Handle referenced files: if the spec lists files in its References: header, move those files to specs/done-reports/ and update the reference paths in the spec.
4. Commit your changes with a commit message starting with 'finalize: ' followed by a brief description.
