...
#   --template <image>       Container image to use (default: auto-selected based on --agent)
...
AGENT="claude"
TEMPLATE_CLAUDE="registry:5000/docker-sandbox-buildgit:latest"
TEMPLATE_CODEX="registry:5000/codex-sandbox-buildgit:latest"
TEMPLATE=""
NAME_OVERRIDE=""
DIR=""
...
# Default template based on agent if not overridden by --template
if [[ -z "$TEMPLATE" ]]; then
    if [[ "$AGENT" == "codex" ]]; then
        TEMPLATE="$TEMPLATE_CODEX"
    else
        TEMPLATE="$TEMPLATE_CLAUDE"
    fi
fi
